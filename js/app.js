(function(document, window) {
	...
	function createPager(element, parameters) {
		...
		for (var key in previousLinks) {
			...
			link.innerHTML = '<span class="js__page__number visually__hidden">' + previousLinks[key] + '</span>' + key;
		...
		for(var i = startIndex; i < parameters.currentPage + pagesToShow && i <= parameters.pagesNumber; i++) {
			link = document.createElement('a');
			link.href = url + i;
			link.innerHTML = '<span class="js__page__number">' + i + '</span>';
		...
		for (key in nextLinks) {
			...
			link.innerHTML = '<span class="js__page__number visually__hidden">' + nextLinks[key] + '</span>' + key;
			var listItem = document.createElement('li');
			listItem.appendChild(link);
			element.appendChild(listItem);
		}
	...
	function showPhotos(data) {
		createPager(
			document.getElementsByClassName('js__thumbnails__pager')[0], {
				query: lastSearch,
		...
		gallery = new Gallery(data.photos.photo, document.getElementsByClassName('js__gallery__image')[0]);
		gallery.createThumbnailsGallery(document.getElementsByClassName('js__thumbnails__list')[0]);
	}

	function init() {
		document.getElementsByClassName('js__form__search')[0].addEventListener('submit', function(event) {
		...
		var leftArrow = document.getElementsByClassName('js__gallery__arrow__left')[0];
		...
		var rightArrow = document.getElementsByClassName('js__gallery__arrow__right')[0];
		...
		document.getElementsByClassName('js__thumbnails__pager')[0].addEventListener('click', function(event) {
			event.preventDefault();
			var page;
			...
			if (event.target.nodeName === 'SPAN') {
				...
				page = event.target.getElementsByClassName('js__page__number')[0].textContent;
			}

			// Avoid reloading the same page
			if (page && page !== currentLink.getElementsByClassName('js__page__number')[0].textContent) {
